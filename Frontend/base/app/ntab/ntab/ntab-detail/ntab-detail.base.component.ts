import { NtabService } from '../ntab.service';
import { NtabBase} from '../ntab.base.model';
...
import { NtabApiConstants } from '@baseapp/ntab/ntab/ntab.api-constants';
...
export class NtabDetailBaseComponent{
...
confirmationReference:any;
  ...
  "actions" : {
	...
	pageViewTitle: string = 'NTAB_DETAIL';
	
	public ntabService = inject(NtabService);
...
public location = inject(Location);
  ...
  const requestedObj = this.generateRequestObj(data);
      ...
      const saveSubscription = this.uploadAttachmentsandSaveData(requestedObj, splittedData,actionName).subscribe(res => {
        ...
        const saveSubscription = this.ntabService[method](requestedObj,actionName).subscribe((res:NtabBase) => {
          this.onAfterSave(res, data, method, isToastNotNeeded);
        })
        this.subscriptions.push(saveSubscription);
      }
...
uploadAttachmentsandSaveData(data: any, splittedData: any,actionName?:string): Observable<any> {
    ...
    if (!this.id) {
       const saveSubscription =  this.ntabService.create(splittedData.data,actionName).subscribe(
        createdData => {
    ...
    const subject$ = new Subject();    
      const updateSubscription = this.deleteFiles(splittedData).subscribe((responseData: any) => {
        this.saveFiles(responseData).subscribe((dataToUpdate: any) => {
          ...
          this.ntabService.update(dataToUpdate.data,actionName).subscribe(
            res => {
              subject$.next(res);
              subject$.complete();
            },
...
this.backupData = this.appUtilBaseService.deepClone(this.detailFormControls.getRawValue());
    ...
    let action: Exclude<keyof NtabDetailBaseComponent, ' '> = methodName;
    const finalArr: string[] = [];
    this.formErrors = {};
    this.inValidFields = {};
...
showMessage(config:any){
	...
	editFormula(config:any,fieldname:any){
     ...
     if(fieldConfig.formulaMethod){
       let methodName: Exclude<keyof NtabDetailBaseComponent, ' '> = fieldConfig.formulaMethod
...
workflowInformation.step = workflowDataForNewRec.step || '';
  ...
  if (environment.prototype && this.id) {
    ...
    this.ntabService.getProtoTypingDataById(params).subscribe((res: any) => {
    ...
    const dataSubscription = this.ntabService.getById (params).subscribe((res: NtabBase[]) => {
      this.data = res || {};
      this.formatRawData();
    });
