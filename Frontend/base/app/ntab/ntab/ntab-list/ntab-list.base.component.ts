...
export class NtabListBaseComponent{
...
localStorageStateKey = "ntab-list";
...
exportTableName: string ="Ntab";
  ...
  "showGridlines" : false,
  "detailPage" : {
    "sid" : "7362f8c7-e03e-46bf-af35-ef2b37731122",
    "name" : "Ntab Detail",
    "url" : "/ntab/ntabdetail"
  },
  "detailPageNavigation" : "click_of_the_row"
}
	pageViewTitle: string = 'NTAB_LIST';
...
public location = inject(Location);
  ...
  if (formControl.controls[parent] instanceof FormGroup){
    ...
    return {
      ...
      ajaxUrl: NtabApiConstants.getDatatableData,
      select: true,
...
getParentId() {
  ...
  let property: Exclude<keyof NtabListBaseComponent, ' '> = value;
  if (this.isChildPage) {
    if (this[property]) {
  ...
  this.clearFilterValues();
	...
	if (this.isChildPage && this[property]) {
		...
		let action: Exclude<keyof NtabListBaseComponent, ''> = methodName;
		if (typeof this[action] == "function") {
			this[action]();
		}
	}
	else {
		this.router.navigate(['../ntabdetail'], { relativeTo: this.activatedRoute});
	}
}
	filterSearch() {
 ...
 if(!(this.appUtilBaseService.isEqualIgnoreCase(value,this.filter.quickFilter, [],true))){
    ...
    if (environment.prototype && this.tableConfig.children?.length > 0) {
      gridSubscription = this.ntabService.getProtoTypingData().subscribe((data: any) => {
...
this.location.back();
    ...
    if (this.selectedValues.length > 0) {
      ...
      this.confirmationService.confirm({
        ...
        accept: () => {
          const deleteSubscription = this.ntabService.delete(requestedParams).subscribe((res: any) => {
