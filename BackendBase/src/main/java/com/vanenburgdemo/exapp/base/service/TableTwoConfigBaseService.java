...
public class TableTwoConfigBaseService<T extends TableTwoConfigBase> extends BaseBusinessLogic<T>
		implements ITableTwoConfigBaseService<T> {
		
	private static final Logger LOGGER = LoggerFactory.getLogger(TableTwoConfigBaseService.class.getName());
	...
	@Autowired
	private  TableTwoConfigPerimeterBaseImpl tabletwoconfigPerimeterBaseImpl;
	
	
	public TableTwoConfigBaseService(Class<T> modelClass) {
		super(modelClass);
		...
		registerModelTransformer(PersistenceType.ANALYTICAL, new TableTwoConfigAnalyticalTransformer());
	}
	...
	public void isObjectExists(T modelObj, boolean isUpdate) {
		T existing = getByField("embed", modelObj.getEmbed());
		
		if(isUpdate) {			
			if(existing == null || existing.getSid().equals(modelObj.getSid())) {
				return;
			}
			throw new AlreadyExistsException(ErrorCode.ALREADY_EXISTS, "TableTwoConfig", modelObj.getEmbed());
	...
	public void onAfterSaveDB(T modelObj) {
	  changelogService.createChangeLog("TableTwoConfig", modelObj.getSid(), Constants.SAVED, modelObj);
	}
	...
	public void onAfterUpdateDB(T modelObj) {
	  changelogService.createChangeLog("TableTwoConfig", modelObj.getSid(), Constants.UPDATED, modelObj);
	}
	...
	public void onAfterDeleteDB(T modelObj) {
	  changelogService.createChangeLog("TableTwoConfig", modelObj.getSid(), Constants.DELETED, modelObj);
	}	
	...
	public T getByEmbed(String embed) {
		List<Filter> filters = new ArrayList<>();
		filters.add(new SimpleFilter("embed", embed, Operator.EQUAL));
	...
	protected IPerimeterManager<T> getPerimeterManager() {
		return tabletwoconfigPerimeterBaseImpl;
	}
}