...
public class TableOneConfgBaseService<T extends TableOneConfgBase> extends BaseBusinessLogic<T>
		implements ITableOneConfgBaseService<T> {
		
	private static final Logger LOGGER = LoggerFactory.getLogger(TableOneConfgBaseService.class.getName());
	...
	@Autowired
	private  TableOneConfgPerimeterBaseImpl tableoneconfgPerimeterBaseImpl;
	
	
	public TableOneConfgBaseService(Class<T> modelClass) {
		super(modelClass);
		...
		registerModelTransformer(PersistenceType.ANALYTICAL, new TableOneConfgAnalyticalTransformer());
	}
	...
	public void isObjectExists(T modelObj, boolean isUpdate) {
		T existing = getByField("value1", modelObj.getValue1());
		
		if(isUpdate) {			
			if(existing == null || existing.getSid().equals(modelObj.getSid())) {
				return;
			}
			throw new AlreadyExistsException(ErrorCode.ALREADY_EXISTS, "TableOneConfg", modelObj.getValue1());
	...
	public void onAfterSaveDB(T modelObj) {
	  changelogService.createChangeLog("TableOneConfg", modelObj.getSid(), Constants.SAVED, modelObj);
	}
	...
	public void onAfterUpdateDB(T modelObj) {
	  changelogService.createChangeLog("TableOneConfg", modelObj.getSid(), Constants.UPDATED, modelObj);
	}
	...
	public void onAfterDeleteDB(T modelObj) {
	  changelogService.createChangeLog("TableOneConfg", modelObj.getSid(), Constants.DELETED, modelObj);
	}	
	...
	public T getByValue1(String value1) {
		List<Filter> filters = new ArrayList<>();
		filters.add(new SimpleFilter("value1", value1, Operator.EQUAL));
	...
	protected IPerimeterManager<T> getPerimeterManager() {
		return tableoneconfgPerimeterBaseImpl;
	}
}